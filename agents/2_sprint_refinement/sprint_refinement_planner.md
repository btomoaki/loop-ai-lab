...
1. **Decision #2: PNG Only (Strictly No SVG / Custom Renderers)**:
   - Output format is strictly `image/png` (250px × 250px).
   - **STRICTLY PROHIBIT SVG generation or custom vector renderers**. Use Go standard library `image`, `image/png`, `image/color`, and `crypto/md5` only.
...
4. **Security & Governance**:
   - Explicit rate limiting middleware returning `HTTP 429`.
   - Docker container execution as non-root user (UID `65532`).
...
- **[Architect Persona]**: Designs Go backend package layout, pure functions, and domain interfaces using standard library (`image/png`).
...
- **[Capacity Guardian Persona]**: **AI Model Expert**. Actively intervenes in the debate to challenge oversized tasks (>=8 SP), strictly bans bloated frameworks/SVG renderers, and forces immediate decomposition into sub-tasks (1-5 SP) for the downstream Coder model.
